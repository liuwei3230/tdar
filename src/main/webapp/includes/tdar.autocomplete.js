...
//		console.log(item);
		...
		var htmlSnippet = "<p style='min-height:4em'><img class='silhouette' src=\""
				...
				+ "<span class='name'>"
				+ htmlEncode(htmlEncode(item.properName))
				+ "("
				+ htmlEncode(htmlEncode(item.email))
				+ ")</span><br/><span class='institution'>"
				+ htmlEncode(htmlEncode(institution))
				+ "</span></p>";
...
//			console.log(item);
			...
			//double-encode on custom render
			return $("<li></li>").data("item.autocomplete", item).append(
					"<a  title=\"" + htmlDecode(description) + "\">" + htmlEncode(htmlEncode(item.value)) + link
							+ "</a>").appendTo(ul);
		};
