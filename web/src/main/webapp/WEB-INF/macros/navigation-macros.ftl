...
<#--        <#else>
    ...
    <#macro clearDeleteButton id="" disabled=false title="delete this item from the list" addon=false>
    <button class="btn btn-mini <#if addon>add-on</#if> repeat-row-delete" type="button" tabindex="-1" title="${title}" <#if disabled> disabled="disabled"</#if>><i class="icon-trash"></i></button>
    </#macro>
