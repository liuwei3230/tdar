...
jQuery.extend({
     ...
     * @param arrayB
     * @param ignoreOrder
     *            if true, ignore order of the array contents (optional: default true)
 ...
 * 
 * Some forms fields allow the user to create a new resource in a child window. This function allows caller to specify the element on the parent window. When
 * the user completes the child form, the tdar will update the target form field with the ID/name of the resource that the user created in the child window.
 * 
 * @param elem
 *            context root
 * @param selector
 *            jqselector which contains the target (optional: default "div")
 */
...
/**
 * Populate a coding sheet / ontology field (aka the adhoctarget) with the id/name of the object created via the child page.
 * 
 * Note: tdar cannot handle multiple, simultaneous adhoc child windows (though this is unlikely to happen)
 * 
 * @param obj
 *            jsobject with id + title properties
 * 
 */
 ...
 * 
 * @param property
 *            name of the property in an object to evaluate when comparing two objects.
 * @param caseSensitive
 *            true if the sort function should be case sensitive (optional: default false)
...
/**
 * Suppress the browser's default behavior of submitting the current form when user presses RETURN while a text-input has focus. User can still submit via
 * keypress when focussed on a submit button.
 * 
 * @param $form
 *            jQuery selection containing the form that will suppress keypress submissions.
 */
...
/**
 * Perform initialization and setup for a typical elements and functionality of a tdar "edit page". This does not include initialization tasks for specific edit
...
//  $('[data-toggle="popover"]').popover();
  ...
  $("[data-tooltip]").each(function(i,el) {
    ...
    $('#resourceCollectionTable').on("focus", ".collectionAutoComplete", function() {
        autocomplete.applyCollectionAutocomplete($(this), {
            showCreate : true,
            showCreatePhrase : "Create a new collection"
        }, {
...
/**
 * Perform initialization tasks for a typical tdar "view" page. Elements and functionality that are unique to a specific page are not addressed by this funtion.
 */
...
var _initRightsPage = function() {
    $('#sharesTable').on("focus", ".collectionAutoComplete", function() {
        ...
        autocomplete.applyCollectionAutocomplete($(this), {
            ...
            permission : "ADD_TO_SHARE"
        });
    });
}

/**
 * Custom ajax filter (enable by calling $.ajaxPrefilter(_customAjaxPrefilter). JQuery executes this prefilter prior to any ajax call.
 * 
 * @param options
 *            options for the current request (including jquery defaults)
 * @param originalOptions
 *            options passed to $.ajax() by the caller, without defaults.
 * @param $xhr
...
/**
 * Register event listener that displays generic wait message for ajax requests. If the ajaxOptions property of the event contain a "waitmessage" property,
 * display that messages, otherwise the function displays "Loading" while the request is in flight, and "Done" after the request is complete.
 */
...
var _determineResponsiveClass = function(width) {
    return width > 1200 ? 'responsive-large-desktop' : width > 979 ? 'responsive-desktop' : width > 767 ? 'responsive-tablet'
            : width > 500 ? 'responsive-phone' : width > 1 ? 'responsive-phone-portrait' : '';
}
 ...
 * @param {...string}
 *            replacements.
 * @returns {*} string containing replacements (if provided). for example, sprintf("{0} {0} {0} your {1}, gently down the stream", "row", "boat");
 */
...
/**
 * After certain amount of time, display a dialog indicating that the users session has expired, then direct the window to the login page.
 */
var _sessionTimeoutWarning = function() {
    ...
    if (parseInt(currentTime)) {
        ...
        if (remainingTime == 300) {
            var dialog = $('<div id=timeoutDialog></div>').html(
                    "<B>Warning!</B><br/>Your session will timeout in 5 minutes, please save the document you're currently working on").dialog({
                modal : true,
 ...
 *            element/selector
 * @param container
 *            element/selector. Context for .typeToggle search.
 */
...
/**
 * Similar to switchType, but this (i think)) swaps out labels and descriptions for inputs that are re-used by multiple document types.
 * 
 * @param el
 *            doctype select element
 * @private
 */
 ...
 *            total number of flies that can be associated with the resource
 * @param rtype
 *            resource type name
 * @private
 */
...
/**
 * Used by ontology and coding-sheet edit pages; show relevant fields based on users choice of "manual text entry" or "file upload"
 */
 ...
 *            size in bytes
 * @param si
 *            true if description should be in SI units (e.g. kilobyte, megabyte) vs. IEC (e.g. kibibyte, mebibyte)
...
/**
 * Initialize bootstrap galleries that have .image-carousel class. Bootstrap Gallery typically doesn't require initialization, but we do extra stuff such as
 * lazy-loading of thumbnails for big lists, and binding of analytics events.
 * 
 * @private
 */
...
function _checkWindowSize() {
    ...
    $(document.body).removeClass('responsive-large-desktop responsive-desktop responsive-tablet responsive-phone responsive-phone-portrait')
            .addClass(new_class);
}
